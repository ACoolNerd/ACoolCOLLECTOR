...
create index if not exists promotion_entries_campaign_user_idx
  on public.promotion_entries(promotion_campaign_id, user_id, created_at);
...
) as v(category_slug,slug,display_name,publisher_or_brand,official_url)
...
on conflict (franchise_id, product_code, release_date) do update set
  name=excluded.name,
...
from events e
where not exists (
  select 1 from public.card_shows c where c.name=e.name and c.starts_at::date=e.start_date
);

with events(name, start_date, ticket_url) as (values
  ('Collect-A-Con New Jersey','2026-07-11'::date,'https://www.universe.com/events/collect-a-con-new-jersey-tickets-2PGRNS'),
  ('Collect-A-Con Minneapolis','2026-07-18'::date,'https://www.universe.com/events/collect-a-con-minneapolis-mn-tickets-753L6V'),
  ('Collect-A-Con Los Angeles','2026-08-01'::date,'https://www.universe.com/events/collect-a-con-los-angeles-ca-tickets-W0MHL8'),
  ('Collect-A-Con San Antonio','2026-08-15'::date,'https://www.universe.com/events/collect-a-con-san-antonio-tx-tickets-G89NQR'),
  ('Collect-A-Con Charlotte','2026-08-22'::date,'https://www.universe.com/events/collect-a-con-charlotte-nc-tickets-VK13B0'),
  ('Collect-A-Con Richmond','2026-08-29'::date,'https://www.universe.com/events/collect-a-con-richmond-va-tickets-6NK4R3'),
  ('Collect-A-Con San Francisco','2026-09-12'::date,'https://www.universe.com/events/collect-a-con-san-francisco-ca-tickets-2L05J4'),
  ('Collect-A-Con Atlanta 2','2026-09-26'::date,'https://www.universe.com/events/collect-a-con-atlanta-2-ga-tickets-WHVL4T'),
  ('Collect-A-Con Chicago 2','2026-10-10'::date,'https://www.universe.com/events/collect-a-con-chicago-2-il-tickets-63HX4L'),
  ('Collect-A-Con Dallas','2026-10-24'::date,'https://www.universe.com/events/collect-a-con-dallas-tx-tickets-13NKP0'),
  ('Collect-A-Con Houston 2','2026-11-07'::date,'https://www.universe.com/events/collect-a-con-houston-2-tx-tickets-FXZ3PL'),
  ('Collect-A-Con New Jersey 2','2026-11-21'::date,'https://www.universe.com/events/collect-a-con-new-jersey-2-tickets-CK5907'),
  ('Collect-A-Con Los Angeles 2','2026-12-19'::date,'https://www.universe.com/events/collect-a-con-los-angeles-2-ca-tickets-9NVCZT')
)
...
from events e
join public.card_shows c on c.name=e.name and c.starts_at::date=e.start_date
...
create policy catalog_sources_read on public.catalog_sources for select using (enabled=true);
drop policy if exists categories_read on public.collectible_categories;
create policy categories_read on public.collectible_categories for select using (active=true);
drop policy if exists franchises_read on public.franchises;
create policy franchises_read on public.franchises for select using (active=true);
drop policy if exists catalog_sets_read on public.catalog_sets;
create policy catalog_sets_read on public.catalog_sets for select using (status <> 'rumored');
drop policy if exists catalog_products_read on public.catalog_products;
create policy catalog_products_read on public.catalog_products for select using (true);
drop policy if exists checklist_read on public.set_checklist_items;
create policy checklist_read on public.set_checklist_items for select using (true);
drop policy if exists ticket_offers_read on public.event_ticket_offers;
create policy ticket_offers_read on public.event_ticket_offers for select using (true);

drop policy if exists event_plans_owner_all on public.user_event_plans;
create policy event_plans_owner_all on public.user_event_plans for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists savings_goals_owner_all on public.savings_goals;
create policy savings_goals_owner_all on public.savings_goals for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists savings_contributions_owner_all on public.savings_contributions;
create policy savings_contributions_owner_all on public.savings_contributions for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists collection_goals_owner_all on public.collection_goals;
create policy collection_goals_owner_all on public.collection_goals for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists collection_goal_items_owner_all on public.collection_goal_items;
create policy collection_goal_items_owner_all on public.collection_goal_items for all
  using (exists (select 1 from public.collection_goals g where g.id=collection_goal_id and g.user_id=auth.uid()))
  with check (exists (select 1 from public.collection_goals g where g.id=collection_goal_id and g.user_id=auth.uid()));
drop policy if exists user_deck_goals_owner_all on public.user_deck_goals;
create policy user_deck_goals_owner_all on public.user_deck_goals for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists bargain_sessions_owner_all on public.bargain_bin_sessions;
create policy bargain_sessions_owner_all on public.bargain_bin_sessions for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists bargain_items_owner_all on public.bargain_bin_items;
create policy bargain_items_owner_all on public.bargain_bin_items for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists recommendation_runs_owner_all on public.recommendation_runs;
create policy recommendation_runs_owner_all on public.recommendation_runs for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists recommendation_items_owner_read on public.recommendation_items;
create policy recommendation_items_owner_read on public.recommendation_items for select
  using (exists (select 1 from public.recommendation_runs r where r.id=recommendation_run_id and r.user_id=auth.uid()));
drop policy if exists public_promotions_read on public.promotion_campaigns;
create policy public_promotions_read on public.promotion_campaigns for select
  using (published=true and status in ('approved','open','closed','draw_pending','drawn'));
drop policy if exists promotion_entries_owner_all on public.promotion_entries;
create policy promotion_entries_owner_all on public.promotion_entries for all using (user_id=auth.uid()) with check (user_id=auth.uid());
drop policy if exists experiment_assignments_owner_read on public.experiment_assignments;
create policy experiment_assignments_owner_read on public.experiment_assignments for select using (user_id=auth.uid());
drop policy if exists experiment_events_owner_insert on public.experiment_events;
create policy experiment_events_owner_insert on public.experiment_events for insert with check (user_id=auth.uid() or user_id is null);
